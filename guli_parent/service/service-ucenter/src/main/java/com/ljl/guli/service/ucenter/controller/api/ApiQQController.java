...
import com.ljl.guli.service.ucenter.utils.UcenterQQPropertites;
...
@Controller
@RequestMapping("/api/ucenter/qq")
//@CrossOrigin
@Api(description = "qq登录")
@Slf4j
public class ApiQQController {

    @Autowired
    private UcenterQQPropertites ucenterQQPropertites;
    ...
    public String callback(String code, String state) {
        try {
            ...
            String baseAccessTokenUrl = "https://graph.qq.com/oauth2.0/token" +
                    "?client_id=%s" +
                    "&client_secret=%s" +
                    "&code=%s" +
            ...
            String accessTokenUrl = String.format(
                    baseAccessTokenUrl,
                    ucenterQQPropertites.getAppId(),
                    ucenterQQPropertites.getAppSecret(),
                    code
            );
            ...
            if(member == null) {//memeber是空，表没有相同微信数据，进行添加
                ...
                String avatar = (String)userInfoMap.get("imgurl");//头像
    ...
    public String genQrConect(HttpSession session) {
        ...
        String encode = "";
        try {
            encode = URLEncoder.encode(ucenterQQPropertites.getRedirectUri(), "UTF-8");
        ...
        String qrCodeUrl = String.format(baseUrl, ucenterQQPropertites.getAppId(), encode, state);
        //跳转到url地址
